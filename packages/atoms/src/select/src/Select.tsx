...
import { useProviderProps, useFocus, useCombinedRefs } from '@jenga-ui/core'
import {
  Element,
  extractStyles,
  BLOCK_STYLES,
...
function Select<T extends object>(
  ...
  let { triggerRef, popoverRef, listBoxRef, inputStyles } = props
...
function Option({ item, state, styles, shouldUseVirtualFocus }: any) {
  const ref = useRef<HTMLDivElement>(null)
  const isDisabled = state.disabledKeys.has(item.key)
  const isSelected = state.selectionManager.isSelected(item.key)
  const isVirtualFocused = state.selectionManager.focusedKey === item.key
  ...
  const { focusProps } = useAriaFocus({ onFocusChange: setFocused })
