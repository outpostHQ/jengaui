...
import { Form, Field, Submit, Input, DialogForm } from '@jenga-ui/core'
import { DialogFormApp } from './components/DialogFormApp'
import { DISABLE_BASE_ARGS } from '@jenga-ui/form'
...
Jenga UI Kit Forms are designed after AntD Forms and [rc-field-form](https://field-form-react-component.vercel.app/) and have very similar API.
...
| setFields       | Set fields status                          | (fields: JengaFieldData[]) => void                       |
...
```tsx
import { Form, Field, Submit, Input } from '@jenga-dev/ui-kit'
...
export const ConfirmDeletionDialogForm = forwardRef(
  (
    ...
    ref: ForwardedRef<JengaDialogFormRef>
  ) => {
...
import { JengaDialogFormRef, Button } from '@jenga-dev/ui-kit'
import { ConfirmDeletionDialogForm } from './ConfirmDeletionDialogForm'

export function DialogFormApp() {
  const ConfirmDeletionDialogFormRef = useRef<JengaDialogFormRef>(null)
